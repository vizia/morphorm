...
fn absolute_layout_align_items_center() {
    ...
    world.set_top(node0, Units::Stretch(1.0));
    world.set_bottom(node0, Units::Stretch(1.0));
    ...
    assert_eq!(world.cache.posx(node0), 0.0);
    assert_eq!(world.cache.posy(node0), 30.0);
}